...
public class MazeSolver {
	...
	public MazeSolver(Maze maze) {
		...
		altura = maze.getAltura();
		largura = maze.getLargura();
		// Copia completa usando stream
		solvedGrid = Arrays.stream(maze.getMazeGrid())
						.map(String[]::clone)
						.toArray(String[][]::new);
		
		
		// Esses cells será usado para todas os elementos do grid
		createdCells = new Cell[altura][largura]; 
		initCells();
		solveMaze();
	}
	...
	private void solveMaze() {
		// Acha as coordenadas de entrada e saída
		int exit = findExitPoint();
		
		// Procura. Cada celula passada é colocada no Stack
		...
		while (true) {
			...
			if (selected.getX() == exit && selected.getY() == largura - 1) {
				// Se a última célula pega foi o final, encerra o programa
				break;
			}
			
			// Pega o vizinho mais importante.
			Cell next = getValidNeighbour(selected);
			if (next != null) {
				next.setInPath(true);
				next.setVisited(true);
				pathFound.offerLast(next);
			} else {
				// Não há vizinhos válidos. Volta uma celula
				selected.setInPath(false);
				pathFound.pollLast();
			}
			
			if (pathFound.isEmpty()) {
				System.out.println("Não há saída nesse labirinto");
				return;
			}
		}
	...
	private int findExitPoint() {
		for (int i = 0; i < largura; i++) {
			if (solvedGrid[i][largura - 1].equals("  ")) {
				return i;
			}
		}
		return 0;
	...
	// Popular as células. Dessa vez todos os elementos do grid serão usados
	...
	private void populateCellsNeighbour() {
		for (int i = 0; i < altura; i++) {
			for (int j = 0; j < largura; j++) {
				...
				// Pega todos os vizinhos dessa celula
	...
	private Cell getValidNeighbour(Cell selected) {
		// Tentativas na ordem: direita > baixo > cima > esquerda
	...
	public String toString() {
		return "MazeSolver [maze=" + maze + ", altura=" + altura + ", largura=" + largura + "]";
	}
}
